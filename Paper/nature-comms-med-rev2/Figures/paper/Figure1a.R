...
fig1ainset <- ggpubr::get_legend(fig1ainset)

fig1ayaxis <- grid::textGrob("Circumcision coverage (%)", rot = 90, gp = gpar(fontsize=9, fontface = "bold"))

fig1a <- wrap_elements(full = get_geofacet_grob(fig1ap)) +
  inset_element(fig1ainset, 0, 0.81, 0.31, 0.92, align_to = "full", on_top = TRUE) +
  inset_element(fig1ayaxis, 0.07, 0.4, 0.09, 0.7)

# Saving plot
ggsave("Figure1a.pdf", fig1a, height = 7.5, width = 7.5, units = "in")
ggsave("Figure1a.png", fig1a, height = 7.5, width = 7.5, units = "in")
ggsave("Figure1a.eps", fig1a, height = 7.5, width = 7.5, units = "in", device = cairo_ps)
save(fig1a, file = "Figure1a.RData")
