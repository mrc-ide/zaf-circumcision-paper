...
results_agegroup <- results_agegroup[,c('area_id','area_name', 'area_level', 'age_group', 
                                        'model', 'type','year','mean', 'lower', 'upper')]
results_agegroup$age_group <- factor(results_agegroup$age_group, 
...
results_survey <- results_survey %>%
  mutate(type = recode(indicator, "circumcised " = "Circumcision coverage (MC)"))
results_survey$age_group <- factor(results_survey$age_group, 
                                   levels = c(  '0-4',   '5-9', '10-14', '15-19', '20-24', '25-29', 
                                                '30-34', '35-39', '40-44', '45-49', '50-54', '55-59'))
...
save(fig1cp, file = "Figure1c.RData")
