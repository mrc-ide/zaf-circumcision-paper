...
######################################################
### Line plot for single age circumcision coverage ###
######################################################
# List of coverage estimates
test1 <- c('Circumcision coverage (MC)', 'Circumcision coverage (MMC)', 'Circumcision coverage (TMC)')
test2 <- c('MC', 'MMC', 'TMC')
...
for (i in 1:length(test1)){
  ...
  p1 <- ggplot(tmp1, 
    ...
    scale_y_continuous(breaks = scales::pretty_breaks(6), limits = c(0, 100)) + 
  ...
  # Outputting plot
  pdf(paste('Coverage/', test2[i], 'coverage_SingleAge_Province.pdf', sep = ''), width = 12, height = 9)
  print(p1)
  dev.off()
}
...
for (i in 1:length(test1)){
  ...
  p1 <- ggplot(tmp1, 
    ...
    geom_point(data = expand.grid(age = 0,
               ...
               colour = 'white') +
    # Adding target line to prevalence
    geom_hline(data = expand.grid(age = 0,
                                  area_id = unique(tmp1$area_id),
                                  year = 2008,
                                  mean = c(80)),
               aes(yintercept = mean),
    ...
    theme(axis.text = element_text(size = 14),
          ...
          legend.position = 'bottom')
  # Outputting plot
  pdf(paste('Coverage/', test2[i], 'coverage_SingleAge_District.pdf', sep = ''), width = 24, height = 21 )
  print(p1)
  dev.off()
}
