...
\begin{figure}[H]
	...
	\begin{tikzpicture}[node distance=2cm]
		\draw [black, fill=gray!30] (-7.5cm,-1.75cm) -- (5.25cm,-1.75cm) -- (5.25cm,-7.25cm) -- (-7.5cm,-7.25cm) -- cycle;
		% Nodes 
		\node (MC) [bigrec2] {Survey data};
		\node (MMC) [startstop, below of = MC, right of = MC, xshift = 1cm, yshift = -1cm] {MMC-nT \\ $\lambda^{\text{MMC-nT}}$};
		\node (TMIC) [startstop, below of = MC, left of = MC, xshift = -1cm, yshift = -1cm] {TMIC \\ $\lambda^{\text{TMIC}}$};
		\node (TMC) [startstop, below of = TMIC, left of = TMIC, xshift = -0.25cm, yshift = -1cm] {TMC \\ $(1-p)\cdot\lambda^{\text{TMIC}}$};
		\node (MMCT) [startstop, below of = TMIC, right of = TMIC, xshift = 0.25cm, yshift = -1cm] {MMC-T \\ $p\cdot\lambda^{\text{TMIC}}$};
		\node (ProgData) [bigrec, below of = MC, xshift = 1.25cm, yshift = -6.75cm] {VMMC programme data};
		% Arrows
	...
	\end{tikzpicture}
    \vspace{0.25cm}
	\caption{{\bf Schematic representation of types of circumcision included in the model.} TMIC = circumcisions conducted as part of traditional male initiation ceremonies; TMC = traditional male circumcisions (conducted by a non-medical practitioner); MMC-T = medical male circumcision conducted as part of traditional male initiation practices; MMC-nT = medical male circumcision conducted outside the context of traditional male initiation (the majority of VMMC for HIV prevention and other routine medical male circumcision). The parameters $\lambda^{\text{MMC-nT}}$ and $\lambda^{\text{TMIC}}$ represent the probabilities of having an MMC-nT and TMC respectively and $p$ represent the proportion of TMICs conducted as medical methods.}
	\label{fig::modelstr}
\end{figure}
