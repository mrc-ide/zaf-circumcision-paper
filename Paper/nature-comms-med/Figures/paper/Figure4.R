...
fig4p <- tmp %>%
  ...
  theme(axis.text.y = element_blank(),
        ...
        axis.text.x = element_text(size = rel(1.1), angle = 50, hjust = 1, vjust = 1.1),
...
fig4inset <- ggpubr::get_legend(fig4inset)
  
  
library(patchwork)

fig4yaxis <- grid::textGrob("Circumcision coverage (%)", rot = 90, gp = gpar(fontsize=9, fontface = "bold"))

fig4 <- wrap_elements(full = get_geofacet_grob(fig4p)) +
  inset_element(fig4inset, 0, 0.81, 0.31, 1.0, align_to = "full", on_top = TRUE) +
  inset_element(fig4yaxis, 0.07, 0.4, 0.09, 0.7)

ggsave("Figure4.pdf", fig4, height = 7.5, width = 7.5, units = "in")
ggsave("Figure4.png", fig4, height = 7.5, width = 7.5, units = "in")
ggsave("Figure4.eps", fig4, height = 7.5, width = 7.5, units = "in", device = cairo_ps)
